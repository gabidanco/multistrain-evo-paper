...
from PCI_Mut_coinf_waning import PCI_Mut_coinf_waning
...
pars['beta'] = np.linspace(0.0, 4.0, pars['nv'])*pars['gamma'] # feasible betas
pars['basR0'] = pars['beta']/(pars['gamma']+ mu)                   # R0 number 
pars['omega'] = (1./(500))*np.ones(pars['nv'])  # waning immunity value
...
plt.savefig('figs/4_EpiCurves_500.pdf')
plt.show()
...
plt.savefig('figs/4_FreqCurves_500.pdf')
plt.show()
#plt.legend()
...
plt.xticks(fontsize = 16)
plt.yticks(fontsize = 16)
plt.savefig('figs/4_NumberVariants_500.pdf')
plt.show()
...
plt.savefig('figs/4_ValuesR0_500.pdf')
plt.show()

### Visualization 5: R0 and frequency at endemic state
It_endemic = It[:,1000:].mean(axis=1)

plt.figure(figsize = (10,8))
plt.plot(pars['basR0'],It_endemic, 'o')
plt.xlabel(r'$R_0$', fontsize = 20)
plt.ylabel(r'$\frac{I_i}{I_T}$', fontsize = 28, rotation = 0, labelpad=16)
plt.xticks(fontsize = 16)
plt.yticks(fontsize = 16)
plt.legend(fontsize = 16)
plt.savefig('figs/4_ValuesR0Freq_500.pdf')
plt.show()
