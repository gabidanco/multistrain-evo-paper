...
# Exploring the effect of coinfection in this model for deterministic beta values 
# Adding the permutations idea 
...
pars['beta'] = np.random.permutation(np.linspace(0.0, 4.0, pars['nv'])*pars['gamma']) # feasible betas
...
plt.savefig('figs/5_EpiCurves.pdf')
plt.show()
...
plt.savefig('figs/5_FreqCurves.pdf')
plt.show()
#plt.legend()
...
plt.xticks(fontsize = 16)
plt.yticks(fontsize = 16)
plt.savefig('figs/5_NumberVariants.pdf')
plt.show()
...
plt.savefig('figs/5_ValuesR0.pdf')
plt.show()
